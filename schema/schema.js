...
const mutation = new GraphQLObjectType({
  ...
  fields: {
    ...
    addWand: {
      ...
      args: {
        // GraphQLNonNull means that this is a necessary argument for this operation
        wizardId: { type: new GraphQLNonNull(GraphQLInt) },
        core: { type: new GraphQLNonNull(GraphQLString) },
        wood: { type: new GraphQLNonNull(GraphQLString) }

      },
      //  the resolve function will always take in the parentValue and the arguments (args above)
      resolve(parentValue, { wizardId, core, wood }) {
        // we are using axios to talk to our basic JSON server sitting in the background
        return axios
          .post("http://localhost:3000/wands", { wizardId, core, wood })
          .then(res => res.data);
      }
    },
  }
